...
Planned work is grouped by priority, not by version number, so plans can move without renumbering. Releases are numbered from v1.0.0 upwards when they ship; any change to a maturity criterion is listed under **Scoring changes** in the [changelog](CHANGELOG.md), so an assessment made under an earlier version can still be compared.
