# Deep Dive: Detection-as-Code and ATT&CK Mapping, Advanced tier
...
> [Running the CDC](cdc-operations.md) §2 states the principles; this document is the engineering manual. Audience: detection engineers and technical leads in S/A-tier CDCs, or teams demanding this of an MSSP. Everything below is vendor-neutral; Sigma is used as the authoring format because it is open and convertible to most platforms.
...
## 2. Anatomy of a rule in Sigma-style YAML

Original example, detecting addition to a privileged directory group:
...
Metadata discipline is the point: the `tags` block is what makes coverage measurable, per §4; the `id` is what makes history traceable, `status` drives pipeline behaviour, and `falsepositives` seeds the runbook. Extend the schema with custom fields if needed, such as owner, review-by date or data-source dependency, but validate them in CI so they stay populated.
...
1. Lint & schema validation: YAML parses; mandatory fields present; UUID unique; tags reference valid ATT&CK IDs, validated against the ATT&CK STIX data rather than a hand-maintained list.
...
3. Conversion: compile Sigma to the target platform's query language with sigma-cli or pySigma backends. Conversion errors fail the build; review the compiled output for expensive constructs.
4. Staging deploy: push to a staging or QA workspace and smoke-test against recent live data for volume, so an unexpectedly hot rule gets caught here, not in analysts' queues.
5. Production deploy: API-driven; the pipeline records rule id ↔ platform object id.
6. Drift detection: a nightly job diffs platform state against the repo; console edits are flagged and reverted or back-ported the same week.
7. Emulation validation, scheduled rather than per-commit: periodically execute mapped attack emulations such as Atomic Red Team tests matching each rule's technique tags, in a lab or canary scope and confirm the alert fires end-to-end, through the alert pipeline, not just the query.

Adoption reality: build stages 1–3 first, a week of work with immediate payoff, then 5–6, then automate 7. Do not gate initial adoption on the full pipeline.
...
3. One detection may map to several techniques; several detections per technique is normal and good, since depth beats breadth on high-value techniques.
4. Score coverage per technique on an honesty scale, not a checkbox:
   - 0: no relevant telemetry collected
   - 1: telemetry exists and could be hunted, no detection
   - 2: detection deployed, not validated in the last 12 months
   - 3: detection deployed and validated, having fired in test
5. Scope the matrix before scoring: filter ATT&CK to your threat profile's techniques, per [CTI deep dive](cti-deep-dive.md) §5, and your platforms. "37% of all of ATT&CK" is meaningless; "level ≥2 on 80% of the techniques our top three threat actors use" is a management-grade statement.
6. Generate, don't hand-paint: the pipeline should emit an ATT&CK Navigator layer JSON from the rule tags plus validation results, so the coverage picture is always current. Publish it to the team monthly; bring the threat-profile-scoped view to GOVERN quarterly.
...
% rules with passing unit tests · % rules validated by emulation ≤12 months · median idea→production lead time · drift incidents per month, target zero · expired suppressions, target zero · coverage level ≥2 on threat-profile techniques.
...
- Red Canary, Atomic Red Team, an attack emulation library. https://github.com/redcanaryco/atomic-red-team
- MITRE, *11 Strategies of a World-Class Cybersecurity Operations Center*, 2nd ed.

*Open CDC Framework, licensed CC BY 4.0. ATT&CK® is a registered trademark of The MITRE Corporation.*
