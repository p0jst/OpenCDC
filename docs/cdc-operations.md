# Running the CDC: Tuning, Detection-as-Code, and Team Development

<p class="src" markdown><span class="src-tag practitioner">Practitioner</span>Operating practice from running a CDC.</p>

> The six build documents ([Govern](govern.md) through [Recover](recover.md)) describe *what* to establish. This document describes three operational loops that determine whether the CDC improves or decays after go-live. It targets S-tier and above; E-tier teams apply the tuning loop in simplified form or demand it of their MSSP.
...
Full engineering manual covering repo layout, rule YAML anatomy, pipeline stages and ATT&CK coverage scoring: [Deep dive: detection-as-code](detection-as-code-deep-dive.md).
...
Roles, skills matrices and career paths live in [Roles & competences](roles-and-competences.md); this section is the operating cadence that keeps them real:
...
- One place to work: alerts from every tool land in the central platform, the SIEM or case-management system, with two-way status sync. A tool whose alerts can only be seen in its own console is a silo by construction, and its detections never enter coverage measurement, per [Detection-as-code deep dive](detection-as-code-deep-dive.md) §4.
...
- Exit-aware from purchase: log formats exportable, detections portable through Sigma, per [Detection-as-code deep dive](detection-as-code-deep-dive.md), API-first, on the same portability logic as the MSSP checklist.
